...
function get_job_filters()
{
	...
	$hours_html = '
	  <div class="form-group">
	   ...
	   <option value="all">'.__("Tous les nombres d 'heures",'theme-text-domain').'</option>';
	...
	$countries_html = '
	  <div class="form-group">
	   ...
	   <select class="form-control" id="job-country" name="job-country" onchange="this.form.submit()">
		...
		<option value="all" selected>'.__('Tous les pays','theme-text-domain').'</option>

	   <option value="Usa-plymouth">'.__('Usa-plymouth','theme-text-domain').'</option>
		<option value="Mexico-Guadelajara">'.__('Mexico-Guadelajara','theme-text-domain').'</option>
		
		<option value="usa-Elkmont">'.__('usa-Elkmont','theme-text-domain').'</option>
		<option value="France-Beynost">'.__('France-Beynost','theme-text-domain').'</option>
		<option value="France-Joinville">'.__('France-Joinville','theme-text-domain').'</option>
		<option value="Allemagne">'.__('Allemagne','theme-text-domain').'</option>
		<option value="Espagne">'.__('Espagne','theme-text-domain').'</option>
		<option value="Italie">'.__('Italie','theme-text-domain').'</option>
		<option value="Turquie-Dubullu">'.__('Turquie-Dubullu','theme-text-domain').'</option>
		<option value="Japon-Tokyo">'.__('Japon-Tokyo','theme-text-domain').'</option>
		<option value="Chine-Wuhan">'.__('Chine-Wuhan','theme-text-domain').'</option>
...
add_action( 'add_meta_boxes', 'ville_add_meta_box' );

function ville_html( $post) {
	wp_nonce_field( '_ville_nonce', 'ville_nonce' ); ?>

	<p>
		<label for="ville_ville_du_lieu_de_travail"><?php _e( 'Ville du lieu de travail', 'ville' ); ?></label><br>
		<select name="ville_ville_du_lieu_de_travail" id="ville_ville_du_lieu_de_travail">
			<option <?php echo (ville_get_meta( 'ville_ville_du_lieu_de_travail' ) === '' ) ? 'selected' : '' ?>>--Séléct your options</option>
			<option <?php echo (ville_get_meta( 'ville_ville_du_lieu_de_travail' ) === 'France' ) ? 'selected' : '' ?>>France</option>
			<option <?php echo (ville_get_meta( 'ville_ville_du_lieu_de_travail' ) === 'Anglais' ) ? 'selected' : '' ?>>Anglais</option>
			<option <?php echo (ville_get_meta( 'ville_ville_du_lieu_de_travail' ) === 'Espagniol' ) ? 'selected' : '' ?>>Espagniol</option>
		</select>
	</p><?php
}

function ville_save( $post_id ) {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return;
	if ( ! isset( $_POST['ville_nonce'] ) || ! wp_verify_nonce( $_POST['ville_nonce'], '_ville_nonce' ) ) return;
	if ( ! current_user_can( 'edit_post', $post_id ) ) return;

	if ( isset( $_POST['ville_ville_du_lieu_de_travail'] ) )
		update_post_meta( $post_id, 'ville_ville_du_lieu_de_travail', esc_attr( $_POST['ville_ville_du_lieu_de_travail'] ) );
}
add_action( 'save_post', 'ville_save' );

/*
	Usage: ville_get_meta( 'ville_ville_du_lieu_de_travail' )
*/
